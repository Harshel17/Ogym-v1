...
- **Calorie Analytics:** Weekly and monthly analytics for calorie tracking in the nutrition page. Shows bar chart comparing daily target vs actual intake, with summary stats (average calories, adherence %, days logged). Trainers can optionally set calorie targets when assigning workout cycles to members (stored with `setBy='trainer'` attribution). Goals display "Set by Trainer" badge when trainer-assigned. Analytics API at `/api/nutrition/analytics?period=week|month`.
- **Enhanced Nutrition Page:** Global "+ Add Food" button at top opens category selector modal. Supports Breakfast/Lunch/Dinner/Snack plus dedicated Protein section (tracks shakes, bars, supplements with protein grams required) and Extra Meals with custom labels (e.g., "Post Workout", "Late Night"). Protein intake summary badge shown near calorie ring. Extra meals grouped by label in separate section. Schema extended with `meal_type` enum including 'protein' and 'extra', plus `meal_label` column for custom meal names.
- **Find My Food:** Location-based restaurant finder that suggests the "safest" meal options based on user's calorie goals and remaining calories for the day. Uses OpenStreetMap Overpass API (free, no API key needed) to find nearby restaurants within 1/3/5 mile radius. Matches found restaurants against internal database of 30+ US and Indian chains with goal-specific suggestions (lose/maintain/gain). Features Dika-powered insights at the top of results. Flow: user clicks "Find My Food Nearby" → grants location permission → selects radius → sees restaurants with safest meal suggestions → can tap to log food. Built with feature flag for easy disable. Components: `client/src/components/find-my-food.tsx` (frontend), `server/nutrition/restaurant-suggestions.ts` (chain database), API at `POST /api/nutrition/find-nearby-restaurants`. Tracking events: find_my_food_opened, find_my_food_radius_selected, find_my_food_restaurant_clicked, find_my_food_log_initiated.
...
### Since commit (Jan 31, 2026 - Enhanced Nutrition Page)
```sql
-- Add meal_label column for extra meals
ALTER TABLE food_logs ADD COLUMN IF NOT EXISTS meal_label TEXT;
```
