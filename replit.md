...
- **Enhanced Nutrition Page:** Features a global "+ Add Food" button, supports various meal types (Breakfast, Lunch, Dinner, Snack, Protein, Extra Meals), protein intake tracking, water intake tracking (quick-add buttons: 8oz/12oz/16oz/24oz with daily progress toward 64oz goal), recent foods quick-add for re-logging, and AI-powered nutrition estimation (OpenAI GPT-4o-mini) when food search returns no results. Expanded American food database with 88+ items covering McDonald's, Chick-fil-A, Chipotle, Subway, Taco Bell, Wendy's, Five Guys, Panda Express, Popeyes, Starbucks drinks, grocery staples, and healthy meal prep items.
...
- **Dika Weekly Report:** AI-generated weekly fitness progress reports. Users ask Dika for "weekly report" and get a comprehensive AI analysis of their 7-day data (workouts, nutrition, body measurements, attendance, health data). Reports are stored with unique tokens for public viewing at `/report/:token`. Users can view full reports or email them via Resend. Architecture: `server/dika/weekly-report.ts` (data gathering + AI generation), `WEEKLY_REPORT_DATA` embedded tag pattern (like `MEAL_LOG_DATA`), `WeeklyReportCard` component in dika-drawer, `client/src/pages/weekly-report-page.tsx` for public view. API routes: `GET /api/reports/:token` (public), `POST /api/reports/:token/email` (authenticated).
