...
- **Ask Dika Page Search:** AI-powered search bars on Payments, Members, Attendance, and Trainers pages (owner-only). Natural language queries auto-filter tables to matching results. Endpoints: `/api/owner/ask-dika-payments`, `/api/owner/ask-dika-members`, `/api/owner/ask-dika-attendance`, `/api/owner/ask-dika-trainers`. Each returns `{answer, results, filterNames}` JSON. Single-name results auto-filter the table; multi-name results apply array filter; dismiss clears filters. Payment intelligence uses pre-computed server-side totals to prevent AI calculation errors.
