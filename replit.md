...
- **External Payment Links:** Owners configure payment methods (UPI, Venmo, CashApp, Zelle, PayPal, bank details, custom links) in profile settings. Stored as JSONB `payment_links` column in gyms table with optional `day_pass_price`. Members and kiosk visitors see payment options and click to pay externally, then confirm "I've paid". Payment confirmations tracked in `payment_confirmations` table for owner approval. Components in `client/src/components/payment-settings.tsx`.
