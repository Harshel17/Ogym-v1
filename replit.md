...
- **Gym Intelligence Dashboard (Level 1):** Data-driven insights for owners including Peak Hour Pressure (check-in density by hour with color-coded bar chart), Muscle Trend Intelligence (muscle group usage comparison month-over-month), and Equipment Intelligence (card-based per-equipment analytics with muscle group filtering, click-to-expand detail panels with exercise breakdowns/usage charts/predictions, and trust-building "Planning Overview" replacing aggressive purchase recommendations). Accessible from sidebar and dashboard teaser card. API endpoints at `/api/owner/gym-intelligence/`.
