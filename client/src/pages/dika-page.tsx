...
function MarkdownContent({ content }: { content: string }) {
  const parsed = useMemo(() => {
    let cleanContent = content
      ...
      .replace(/<!-- PENDING_MATCH_LOG:[\s\S]+? -->/g, '').replace(/&lt;!-- PENDING_MATCH_LOG:[\s\S]+? --&gt;/g, '')
      .replace(/\n?\[chips?:\s*"[^"]+".+?\]/gi, '')
      .trim();
...
function DikaPageInner({ userId }: { userId: number }) {
  ...
  useEffect(() => {
    ...
    const isSwap = params.get('swap') === '1';
    const queryKey = `${exerciseName}-${params.get('sets')}-${params.get('reps')}-${params.get('muscle')}-${isSwap ? 'swap' : 'help'}`;
    ...
    let prompt: string;
    if (isSwap) {
      prompt = `I can't do ${exerciseName}`;
      if (muscle) prompt += ` (${muscle})`;
      prompt += ` today. Can you suggest alternative exercises I can swap it with?`;
    } else {
      prompt = `Help me with the exercise "${exerciseName}"`;
      if (muscle) prompt += ` (${muscle})`;
